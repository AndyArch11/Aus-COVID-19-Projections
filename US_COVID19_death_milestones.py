...
def add_mortality_lines(ax, usdf, usmortalitydf, mortality_count_column, mortality_type_column, unit):
    ...
    for index, row in sorted_mortality_df.iterrows():        
        total_deaths_str = '{:,.1f}'.format(row[mortality_count_column])
...
def daily_covid_deaths_plotly(title, usdf, scale, usmortalitydf, mortality_count_column, mortality_type_column, unit):
    ...
    add_mortality_traces(fig, usdf, usmortalitydf, mortality_count_column, mortality_type_column, unit)    
...
def add_mortality_traces(fig, usdf, usmortalitydf, mortality_count_column, mortality_type_column, unit):
    sorted_mortality_df = usmortalitydf.sort_values(by=mortality_count_column, ascending=False)
    for index, row in sorted_mortality_df.iterrows():
        total_deaths_str = '{:,.1f}'.format(row[mortality_count_column])
        if total_deaths_str.endswith('.0'):
